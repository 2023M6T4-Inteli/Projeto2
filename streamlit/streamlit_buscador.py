...
def render_sidebar():
    ...
    # Definindo variáveis de estado para armazenar as seleções
    upload_selected = st.sidebar.button("Upload do dataset")
    dashboard_selected = st.sidebar.button("Dashboard")
    chat_selected = st.sidebar.button("Chat-Btg")
    predicao_selected = st.sidebar.button("Predição")
    
    # Verificando qual botão foi selecionado e retornando a página correspondente
    if upload_selected:
        page = "Upload do dataset"
    elif dashboard_selected:
        page = "Dashboard"
    elif chat_selected:
        page = "Chat-Btg"
    elif predicao_selected:
        page = "Predição"
    else:
        page = "Upload do dataset"  # Página padrão se nenhum botão for clicado
...
if page == "Upload do dataset":
    ...
    upload_file = st.file_uploader('Insira seu arquivo CSV', type='csv')
...
# Mostra a página correspondente à opção selecionada na barra lateral
    ...
    if upload_file_2:
        ...
        pred_button = st.button(label='Predição', key='pred_button_unique_key')
        if pred_button:
